defmodule StarTicketsWeb.Admin.TotemMenusLive do
  ...
  def render(assigns) do
    ...
    <div class="st-app has-background min-h-screen flex flex-col" style="padding-top: 80px;">
      ...
      <div class="st-container flex-1 m-4 overflow-hidden flex flex-col">
            ...
            <%= if @selected_node do %>
               <div class="max-w-2xl mx-auto">
                 <div class="flex items-center justify-between mb-6">
                   <h2 class="text-2xl font-bold text-white"><%= if @selected_node.id, do: "Editar Nó", else: "Novo Nó" %></h2>
                 ...
                 <.form for={@form} phx-change="validate" phx-submit="save" class="space-y-6">
                   ...
                   <%= if @selected_node.id do %>
                     <div class="flex items-center gap-4 my-8">
                        ...
                        <span class="text-white/50 text-sm font-medium uppercase tracking-wider">Serviços Vinculados</span>
                        <hr class="flex-1 border-white/10 border-dashed" />
                     </div>
                     <p class="text-sm text-white/60 mb-4">Se este nó for selecionado, quais serviços estarão disponíveis?</p>
                       ...
                       <%= for service <- @services do %>
                          ...
                          <div class="p-3 rounded border border-white/10 bg-white/5">
                             <label class="flex items-center gap-3 cursor-pointer group">
                                ...
                                />
                                <span class="text-white font-medium group-hover:text-orange-400 transition-colors"><%= service.name %></span>
                             </label>
                   ...
                   <div class="flex justify-end gap-2 mt-6">
                     <button type="button" phx-click="cancel" class="btn btn-ghost text-white/70 hover:text-white">Cancelar</button>
                 ...
                 <%= if @selected_node.id do %>
                   <div class="flex items-center gap-4 my-8">
                      <hr class="flex-1 border-white/10 border-dashed" />
                      <span class="text-white/50 text-sm font-medium uppercase tracking-wider">Ações</span>
                      <hr class="flex-1 border-white/10 border-dashed" />
                   </div>
    ...
    <div class="pl-4 border-l border-white/5 ml-2">
      <div class={"group flex items-center gap-2 py-1.5 px-3 rounded-lg transition-all duration-200 " <> if(@selected_node && @selected_node.id == @menu.id, do: "bg-blue-500/10 border border-blue-500/20 text-blue-200 shadow-lg shadow-blue-500/5 backdrop-blur", else: "text-white/70 hover:bg-white/5 hover:text-white")}>
        ...
        <div class="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
           <button phx-click="move_node" phx-value-id={@menu.id} phx-value-dir="up" class="p-1 hover:bg-white/20 rounded text-white/50 hover:text-white" title="Mover para cima">
             <.icon name="hero-chevron-up" class="w-3 h-3" />
           </button>
           <button phx-click="move_node" phx-value-id={@menu.id} phx-value-dir="down" class="p-1 hover:bg-white/20 rounded text-white/50 hover:text-white" title="Mover para baixo">
