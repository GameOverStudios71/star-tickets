          ...
          <div class="lg:col-span-2 space-y-4">
            <div class="st-acrylic p-6 rounded-xl min-h-[500px]">
              <h3 class="text-white font-semibold mb-4 flex items-center gap-2">
                <.icon name="hero-document-text" class="size-5" /> Preview do Formulário
              </h3>
                    ...
                    <div class="flex justify-between items-start mb-2">
                      ...
                      <div class="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity text-white items-center">
                        <button
                          ...
                          class="p-1 hover:text-yellow-400"
                    ...
                    <div class="pointer-events-none">
                      <%= case field.type do %>
                        <% "text" -> %>
                          <input
                            type="text"
                            class="input input-sm w-full bg-white/5 text-white placeholder-white/50 font-medium"
                        ...
                        <% "textarea" -> %>
                          <textarea
                            class="textarea textarea-sm w-full bg-white/5 text-white placeholder-white/50 font-medium"
                        ...
                        <% "checkbox" -> %>
                          ...
                          <% else %>
                            <div class="flex items-center gap-2">
                              ...
                              <span class="text-gray-200">Sim/Não</span>
                            </div>
                          <% end %>
                        <% "radio" -> %>
                        ...
                        <% "file" -> %>
                          <div class="border border-dashed p-2 text-center text-xs text-gray-300">
                            Upload de Arquivo
                          </div>
                        <% _ -> %>
                          <p class="text-red-400">Tipo desconhecido</p>
                      <% end %>
                    </div>
                  </div>
                <% end %>
            ...
            <div class="st-acrylic p-4 rounded-xl">
              ...
              <div class="grid grid-cols-2 gap-2">
                <button
                  phx-click="add_field"
                  phx-value-type="text"
                  class="btn btn-sm btn-outline text-blue-200 hover:bg-blue-500/20"
                >
                  <.icon name="hero-bars-2" class="mr-1 size-4" /> Texto
                </button>
                <button
                  phx-click="add_field"
                  phx-value-type="number"
                  class="btn btn-sm btn-outline text-green-200 hover:bg-green-500/20"
                >
                  <.icon name="hero-hashtag" class="mr-1 size-4" /> Número
                </button>
                <button
                  phx-click="add_field"
                  phx-value-type="textarea"
                  class="btn btn-sm btn-outline text-purple-200 hover:bg-purple-500/20"
                >
                  <.icon name="hero-document-text" class="mr-1 size-4" /> Área Disp.
                </button>
                <button
                  phx-click="add_field"
                  phx-value-type="checkbox"
                  class="btn btn-sm btn-outline text-yellow-200 hover:bg-yellow-500/20"
                >
                  <.icon name="hero-check-circle" class="mr-1 size-4" /> Checkbox
                </button>
                <button
                  phx-click="add_field"
                  phx-value-type="radio"
                  class="btn btn-sm btn-outline text-orange-200 hover:bg-orange-500/20"
                >
                  <.icon name="hero-stop-circle" class="mr-1 size-4" /> Radio
                </button>
                <button
                  phx-click="add_field"
                  phx-value-type="select"
                  class="btn btn-sm btn-outline text-teal-200 hover:bg-teal-500/20"
                >
                  <.icon name="hero-list-bullet" class="mr-1 size-4" /> Lista
                </button>
                <button
                  phx-click="add_field"
                  phx-value-type="file"
                  class="btn btn-sm btn-outline text-pink-200 hover:bg-pink-500/20"
                >
                  <.icon name="hero-arrow-up-tray" class="mr-1 size-4" /> Upload
                </button>
              </div>
            </div>
          </div>
