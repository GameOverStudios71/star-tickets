defmodule StarTicketsWeb.CoreComponents do
  @moduledoc """
  ...
  use Gettext, backend: StarTicketsWeb.Gettext
  ...
  attr(:kind, :atom,
    values: [:info, :error, :success, :warning, :delete],
  ...
  def flash(assigns) do
    ...
    <div
      ...
      data-kind={@kind}
      role="alert"
      class="toast toast-top toast-end z-50 mt-20"
      {@rest}
    >
      <div class={[
        "alert w-80 sm:w-96 max-w-80 sm:max-w-96 text-wrap shadow-lg border backdrop-blur-md",
        @kind == :info && "bg-blue-500/10 text-blue-100 border-blue-500/20",
        @kind == :success && "bg-emerald-500/10 text-emerald-100 border-emerald-500/20",
        @kind == :warning && "bg-amber-500/10 text-amber-100 border-amber-500/20",
        @kind == :error && "bg-red-500/10 text-red-100 border-red-500/20",
        @kind == :delete && "bg-red-500/10 text-red-100 border-red-500/20"
      ]}>
        <.icon :if={@kind == :info} name="hero-information-circle-solid" class="size-6 shrink-0" />
        <.icon :if={@kind == :success} name="hero-check-circle-solid" class="size-6 shrink-0" />
        <.icon :if={@kind == :warning} name="hero-exclamation-triangle-solid" class="size-6 shrink-0" />
        <.icon :if={@kind == :error} name="hero-exclamation-circle-solid" class="size-6 shrink-0" />
        <.icon :if={@kind == :delete} name="hero-trash-solid" class="size-6 shrink-0" />
        <div>
  ...
  attr(:transparent, :boolean, default: false, doc: "removes default background and padding")
  ...
  def simple_form(assigns) do
    ...
    <.form :let={f} for={@for} as={@as} {@rest}>
      <div class="space-y-4">
        ...
        <div :if={@actions != []} class="flex items-center justify-end gap-6 mt-6">
  ...
  def error(assigns) do
    ~H"""
    <div class="my-2 inline-flex items-center gap-2 px-3 py-1 bg-red-500/50 border border-red-500/20 text-white text-xs font-medium backdrop-blur-sm">
      <.icon name="hero-exclamation-circle" class="size-4 text-white" />
      {render_slot(@inner_block)}
    </div>
    """
  end
  ...
  def translate_error({msg, opts}) do
    ...
    if count = opts[:count] do
      Gettext.dngettext(StarTicketsWeb.Gettext, "errors", msg, msg, count, opts)
    else
      Gettext.dgettext(StarTicketsWeb.Gettext, "errors", msg, opts)
    end
  end
